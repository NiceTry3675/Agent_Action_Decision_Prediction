...
4. Run `[agent]` — starts the `vm_agent.py daemon` (the session's last Quick Pick).
5. From here drive everything locally, no notebook cells needed:
   - launch: `python colab/cloud_sync.py cmd "python colab/vm_agent.py launch train_transformer.py '<args>'"`
...
`@unassign` releases the runtime, `@stop` exits the daemon.
...
- **Idle auto-unassign**: after `AADP_IDLE_MAX_MIN` minutes (default 45) with no live
  training process and no incoming commands, the daemon unassigns the runtime — but
  only after the last run was collected. Idle VMs stop burning CUs on their own.
...
| `cloud_sync.py cmd` times out, `hb` stale | daemon down or VM recycled | rerun `[bootstrap]` + `[agent]`; if hb shows idle unassign fired, start a new runtime |
