# Human Semantic Relabel Report - 2026-07-10
...
- Six primary annotators each read a disjoint blinded chunk. Dataset labels,
  model predictions, logits, and casebook strata were removed.
- Two additional annotators independently reviewed the same 80 priority rows.
  The main reviewer adjudicated the eight rows where both auditors disagreed
  with the primary annotator.
- Labels describe the next action a reasonable coding agent would take from the
  observable state: `list_directory`, `read_file`, `grep_search`,
  `glob_pattern`, `other`, or `underdetermined`.
...
| All three annotators unanimous | 65/80 (81.3%) |
| At least two annotators agree | 80/80 (100%) |
...
annotator applying an idiosyncratic boundary.
...
## Dataset label versus human judgment

| Dataset label | N | Exact human agreement | Dataset label acceptable | Most common human judgments |
| --- | ---: | ---: | ---: | --- |
...
the annotator marked reasonable raised it to 77/248 (31.0%). Even among the 154
human-clear rows, the dataset label agreed on only 37 rows (24.0%).
...
| Current request (abridged) | Dataset / recovered action | Human label |
| --- | --- | --- |
...
1. **Semantic action selection.** A human chooses a tool from the missing
   information: directory layout, known-file contents, content occurrences, or
   path candidates.
...
`list_directory` is the clearest case. In this error sample, it is rarely a
human orientation request; most target rows are semantically content searches
or concrete file reads. Improving semantic understanding alone can therefore
leave `list_directory` F1 unchanged or even reduce it.

## Testable consequences

- Use `human_relabels_final.jsonl` as a diagnostic audit set, not as replacement
  training truth. Public evaluation rewards generator imitation, not human
  correctness.
- Report candidate changes twice on these rows: agreement with dataset truth and
  agreement with the human label. A feature that only improves the former is
  learning generator policy; one that improves the latter is learning semantic
  intent.
- Test a two-part predictor: semantic weak-class logits from `current_prompt`,
  ...
  generator-policy correction. The human labels provide the semantic target for
  analysis, while the original labels remain the optimization target.
