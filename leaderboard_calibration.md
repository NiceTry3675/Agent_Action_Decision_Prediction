...
| 2026-07-04 | len192 focal g2.0 encoder-only val-model, seed42 (`m3_len192_s42.zip`, `...m3_focal_len192_ep5_replay_last1_seed42_valmodel`) | 0.744716 | pending | | length-pair anchor: identical to the len448 arms except max-length. Pure-transfer prediction (no length effect, slope 1.6 off the len448 pair): Public ≈ 0.730. Public clearly above that → len448 adds nothing; clearly below → len448 case survives | transfer-coefficient probe RESULT: fixed delta -0.013 → Public delta -0.021 (transfer ≈ 1.6x, co-moving). Fixed DOES rank same-recipe instances; weak draws are genuinely weak models. Gap for this recipe class -0.013 to -0.021 (grows for weaker instances). Instance selection by fixed score is a valid lever |
