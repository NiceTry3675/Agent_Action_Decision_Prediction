...
| 2026-07-05 | M8 timing probe B: same pack + torch.compile(reduce-overhead) + shipped T4-baked inductor/triton cache, buckets {256,400} b64 (`m8_ep3_cc.zip`) | n/a (same OOF tiers) | **timeout 10:00** | n/a | completion threshold was ~1.85x replica speed — also timed out ⇒ **UPPER BOUND: server speedup s < ~1.85x replica. "훨씬 빠르다" is closed; s ∈ (~1, 1.85) stays OPEN** — marginal paths (cascade ~680s, depth-cut ~650s replica-projected) clear 600s if s ≥ ~1.1. Direct 0.8B with v1 serializer (918s+) is dead at any plausible s; each future marginal-pack submission doubles as an s-measurement |
