...
- Next action: first loop application — rerun the len448-focal fixed screen with weight saving, encoder-only package (`--no-sparse`), submit for the length hypothesis's first Public read (doubles as fixed↔Public calibration point #2 under the new recipe). Watch item: len448 ≈ 2.3x tokens vs len192 against the T4 10-min inference cap.
