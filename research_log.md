...
### 2026-07-10 - GPT-5.6 병렬 에이전트 시맨틱 리레이블: 약클래스 라벨은 생성기 정책 기록, 의미 이해 레인 개념적 종료

- 07-09 약클래스 케이스북의 248개 고유 행을 GPT-5.6 모델 에이전트로 블라인드
  리레이블 (`experiments/artifacts/20260710_human_relabel/`): 1차 에이전트 6개가
  청크를 병렬 처리하고 독립 GPT-5.6 감사 에이전트 2개가 80행을 재검토한 뒤,
  불일치 8행을 별도 GPT-5.6 패스로 판정. **인간 어노테이터는 참여하지 않았으며**
  일치율은 사람 간 신뢰도가 아니라 독립 실행된 동일 모델 에이전트 간 일관성임.
  감사 에이전트 상호 일치 91.3%, 3개 에이전트 만장일치 81.3%인데 데이터셋
  라벨과의 정확 일치는 21.0%, acceptable 허용 시 31.0%. `list_directory`는
  60행 중 정확 일치 1행.
...
- **Decision: 프롬프트 의미 이해 강화 레인(의미 지향 직렬화기, GPT-5.6
  시맨틱 라벨 학습 타깃) 개념적 종료. 레거시 파일명의 GPT-5.6 시맨틱 라벨
  (`human_relabels_final.jsonl`)은 진단 감사 세트로만 사용 — 최적화 타깃은 원
  라벨 유지.** 후보 변경은 이 248행에서 데이터셋 일치/GPT-5.6 시맨틱 라벨
  일치를 이중 리포팅해 정책 학습인지 의미 학습인지 구분. 잔여 헤드룸 탐색은
  생성기 상태(직전 액션·경로·결과 요약) 조건부
...
- **메커니즘 규명(노진산, 교사-학생 분포 전수 대조)**: condalpha의 이득은
  ...
  완충재**. 07-10 GPT-5.6 병렬 에이전트 리레이블 결론과 정합. 보조: solved 4종(edit/write/
...
### 2026-07-11 - gemma 시브 위원 기각 + c=0 만장일치의 정체 규명 (공짜 진단, 슬롯 0)

- 팀 교사 아카이브 train70k 로짓으로 gemma-4-12B의 위원회 적합성을 진단:
  m8과 both-wrong same-alt 93.0%(llama 92.5%와 동급), 오류 10,076행 중 84%가
  이미 c=0에 집중. **Decision: gemma(및 llama) 시브 위원 영입 기각** — 탈상관이
  없어 투표 정보가치가 근사 0, OOF 3폴드(~3 A100h) 비용 불성립.
- c=0 12,776행 구조: 위원회 만장일치 대안 10,449행, 그중 gemma·llama까지
  5모델 교차-가문 만장일치 5,298행. H1(오라벨)/H2(정책 확률성)/H3(공유 절단
  결함) 판별 체크 결과 **H3 기각**(전 행 절단 0%, p90 283토큰) + 만장일치
  행은 오히려 더 짧고 히스토리 중앙값 4 vs 클린 8 — **H2(세션 초반 고엔트로피
  행의 확률적 정책 라벨) 채택.** GPT-5.6 병렬 에이전트 리레이블·정책 혼합 감사·condalpha
  메커니즘 규명과 정합.
- 함의: head-CE 차단 카드 전제 약화(공식 라벨이 소수 질량의 정당한 신호),
  시브 최적점이 "부분 감쇠+head 유지"인 이유와 레인 A/v2 동반 실패가
  메커니즘적으로 설명됨 — 시브 축 포화는 우연이 아님. 수치는
  `experiments/artifacts/20260711_c0_unanimity_gemma_diag.json`.

### 2026-07-11 - 리레이블 재실험(Sonnet 교차-가문): 도크트린 생존, 라벨측 개입 카드 폐기

- 배경: 07-10 "휴먼 시맨틱 리레이블"의 어노테이터가 사람이 아닌 GPT-5.6으로
  확인(사용자 공지) — 상호 신뢰도 91.3%가 동일 모델 반복일 수 있어 재심.
  claude-sonnet 31개 독립 에이전트로 재실험: 원 248행(재현) + 학생-정답
  weak4 대조군 120행(원 실험 미수행) + c=0 5모델 만장일치-대안 120행(H1/H2
  판별), 층 혼합 블라인드, 기준 사전 등록
  (`experiments/artifacts/20260711_sonnet_relabel/`).
- R1 재현됨: Sonnet vs GPT-5.6 79.8%(기준 ≥70%), Sonnet vs 데이터셋 23.4%
  (GPT-5.6 21.0% 재현). 판정은 가문 편향 아님. R2 대조군: 정확 32.5% —
  모델이 맞힌 행에서도 list 5/30·glob 4/30만 의미 판정과 일치(read/grep은
  15/30), 케이스북 전이-prior 통계와 정합: list/glob 라벨은 전역적으로 정책
  기록. R3: 만장일치 대안의 의미적 지지 24.2%뿐, 공식 라벨 acceptable
...
### 2026-07-12 - 오늘 Public 미기록 6장 — 학습축 정리, 챔피언 유지

- `ep4`(4에폭) Public `0.79106`(챔피언 대비 `-0.0028`). 승격 실패;
  에폭 확장 축을 봉인한다.
- `w08`(Weak4 α `0.8`) Public `0.79356`(`-0.0003`). 동률권이지만
  갱신하지 못했으며, `(rest, weak)=(0.5, 0.7)`을 α 격자 정점으로 확정한다.
- `s202` 시드 리롤 Public `0.79377`(`-0.0001`). 챔피언과 실질적으로
  동률이며, 이번 리롤의 실측 시드 변동폭은 약 `0.0001`이다.
- `ema`(가중치 평균) Public `0.79322`(`-0.0007`). 승격 실패;
  가중치-평균 축을 봉인한다.
- `t4`(KD `T=4.0`) Public `0.79258`(`-0.0013`). 승격 실패;
  `T=3.0`을 최적점으로 확정하고 temperature 축을 닫는다.
- `a3f`(ep3 α floor) Public `0.79046`(`-0.0034`). 승격 실패;
  시간축 균일 α-floor 개입을 종료한다.
- **Decision: 위 6장 모두 미승격. 기존 챔피언을 유지하고 해당
  에폭/α 격자/가중치 평균/temperature/시간축을 추가 탐색하지 않는다.**

### 2026-07-12 - 챔피언은 현 레시피 계열의 평평한 국소 plateau — seed 수확 EV 하향

- 오늘 결과는 챔피언 근처의 작은 변경(`s202`, `w08`, `ema`, `t4`,
  `c0a8`)이 동률권 또는 소폭 하락에 머물고, 더 큰/구조적 변경
  (`ep4`, `a3f`, `ccalpha015`, `tm8_sieve`)은 낙폭이 커지는 형태다.
  운영적으로는 챔피언이 넓고 비대칭적인 국소 plateau의 상단에
  가깝다고 판단한다.
- 범위는 **탐색한 KD/시브/노이즈-완충 계열**에 한정한다. `0.002`
  안쪽의 단일 Public 차이로 실제 기울기 0을 증명할 수 없고, 독립적인 정보원을
  추가하는 직교 방향까지 막힌 전역 최적점으로 해석하지 않는다.
- `s202` 리롤이 챔피언과 `-0.0001`에 머문 것은 과거 non-KD HCX의
  `s777 -0.0202`에서 유도한 “넓은 seed 분산 → 높은 best-of-N EV” 사전가정을
  하향 갱신한다. 시브×조건부 α 레시피가 인스턴스 변동도 줄였을 가능성이
  있다. 단, 리롤 한 점은 σ 추정이 아니므로 seed 수확 EV를 0으로 놓지는
  않는다.
