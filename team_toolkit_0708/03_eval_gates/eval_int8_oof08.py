"""Gate the kd_oof08 int8 pack with the server-identical .__scale__ loader.
Mirror of eval_int8_pack.py (s777 memcheck) with oof08 paths + gate settings (20k, pair_pos=-2)."""
import sys
...
INT8 = "/root/dacon/models/kd_oof08_pack/hf_model/model.int8.safetensors"
CFG_DIR = "/root/dacon/models/kd_oof08_refit/hf_model"
...
pairs = second_last_pairs(samples, 20000, -2)
...
with torch.inference_mode():
    for bstart in range(0, len(order), 32):
        ...
        for row, i in zip(torch.argmax(logits, dim=1).tolist(), bidx):
            preds[i] = row
print(f"INT8_PACK_GATE macro_f1={f1_metrics(y_true, preds)['macro_f1']:.6f}")
