"""Export teacher soft-labels: run a trained HF artifact over train.jsonl and save
{ids, logits [N,14] fp16, classes, labels, y_true, metadata} in the KD teacher format
(same schema as teacher_m7m8v6_train70k_fp16.pt)."""
import argparse
import time
...
def main():
    ...
    parser.add_argument("--hf-model", required=True, help="trained hf_model dir")
    ...
    model = AutoModelForSequenceClassification.from_pretrained(args.hf_model, torch_dtype=torch.float16)
    ...
    # length-bucketed batches (sorted order) for fast padding; restore original order after
    ...
    with torch.inference_mode():
        for bstart in range(0, len(bucket), args.batch_size):
            ...
            if done % (args.batch_size * 100) < args.batch_size or done == len(bucket):
                ...
                print(f"  logits {done}/{len(bucket)} elapsed={elapsed:.0f}s", flush=True)

    pred = torch.argmax(logits_out, dim=1)
    acc = float((pred == torch.tensor(y_true)).float().mean())
    print(f"teacher train-set argmax acc={acc:.4f} (memorized-logit sanity, expect high)")
